-- Arreglar la función de validación de horarios para manejar mejor las zonas horarias
...
BEGIN
  ...
  -- ARREGLO: Usar AT TIME ZONE correctamente
  ...
  -- LOG para debugging
  RAISE LOG 'BOOKING VALIDATION: court_type=%, booking_start_hour=%, booking_end_hour=%, operating_start=%, operating_end=%', 
    court_type_name, booking_start_hour, booking_end_hour, operating_start_hour, operating_end_hour;
  RAISE LOG 'BOOKING TIMES: start_time_utc=%, start_time_local=%, end_time_utc=%, end_time_local=%',
    NEW.start_time, booking_local_start, NEW.end_time, booking_local_end;
  
  -- Validar que la reserva esté dentro del horario de operación
  IF booking_start_hour < operating_start_hour THEN
    RAISE EXCEPTION 'Cannot book before operating hours. Operating hours for % start at %:00 (hora local México)', 
      court_type_name, operating_start_hour;
  END IF;
  
  IF booking_end_hour > operating_end_hour THEN
    RAISE EXCEPTION 'Cannot book after operating hours. Operating hours for % end at %:00 (hora local México)', 
