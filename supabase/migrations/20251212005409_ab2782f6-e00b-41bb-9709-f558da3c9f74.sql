-- Modificar el trigger de validación de reservas para que funcione en INSERT y UPDATE
-- y agregar validación de conflictos con special_bookings

-- Primero, eliminar el trigger existente
DROP TRIGGER IF EXISTS booking_validation_trigger ON public.bookings;
DROP TRIGGER IF EXISTS cleanup_before_booking_trigger ON public.bookings;

-- Recrear la función validate_booking con soporte para UPDATE y validación de special_bookings
...
DECLARE
  ...
  special_booking_count integer;
  is_update_operation boolean;
  time_changed boolean;
BEGIN
  -- Detectar si es una operación UPDATE
  is_update_operation := (TG_OP = 'UPDATE');
  
  -- Para UPDATEs, verificar si cambió start_time, end_time o court_id
  IF is_update_operation THEN
    time_changed := (OLD.start_time != NEW.start_time OR OLD.end_time != NEW.end_time OR OLD.court_id != NEW.court_id);
    
    -- Si no cambió la hora ni la cancha, permitir la actualización sin validaciones adicionales
    IF NOT time_changed THEN
      RETURN NEW;
    END IF;
  END IF;

  -- SOLO para INSERT: Cancelar automáticamente cualquier reserva pendiente anterior del mismo usuario
  IF NOT is_update_operation THEN
  ...
  -- VALIDACIÓN DE SPECIAL_BOOKINGS (eventos especiales)
  SELECT COUNT(*) INTO special_booking_count
  FROM public.special_bookings
  ...
  IF special_booking_count > 0 THEN
    RAISE EXCEPTION 'La cancha está reservada para un evento especial en este horario';
  END IF;
  ...
  -- Verificar límite dinámico de reservas activas del usuario (SOLO para INSERT, no UPDATE)
  IF NOT is_update_operation THEN
...
-- Recrear el trigger para INSERT y UPDATE
CREATE TRIGGER booking_validation_trigger
  BEFORE INSERT OR UPDATE ON public.bookings
  FOR EACH ROW
  EXECUTE FUNCTION public.validate_booking();

-- Recrear el trigger de limpieza SOLO para INSERT
CREATE TRIGGER cleanup_before_booking_trigger
