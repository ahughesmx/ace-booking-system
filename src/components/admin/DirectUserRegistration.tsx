...
interface DirectUserRegistrationProps {
  onSuccess?: () => void;
}

export default function DirectUserRegistration({ onSuccess }: DirectUserRegistrationProps) {
  ...
  const onSubmit = async (data: UserRegistrationData) => {
    ...
    try {
      setIsLoading(true);
      console.log("🎯 Starting direct user registration...");
    ...
    <>
      ...
      <Dialog open={showDialog} onOpenChange={(open) => !open && handleCloseDialog()}>
        <DialogContent className="max-w-2xl max-h-[90vh] overflow-y-auto">
          <DialogHeader>
            <DialogTitle>Registro Directo de Usuario</DialogTitle>
          </DialogHeader>
